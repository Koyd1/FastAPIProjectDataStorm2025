...
from app.constants import ORDINAL_ENCODINGS
...
def restore_ordinal_labels(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    for column, mapping in ORDINAL_ENCODINGS.items():
        ...
        def map_value(value: Any) -> Any:
            ...
            try:
                ...
                return value
            return inverse_mapping.get(numeric_value, value)

        restored[column] = restored[column].map(map_value)
    return restored
...
def register_routes(app) -> None:
    ...
    async def predict_single(request: Request, file: UploadFile = File(...)) -> HTMLResponse:
        ...
        preview_dataframe = dataframe.copy(deep=True)
        sanitized_dataframe_raw, clean_notifications = clean_column_values(dataframe.copy(deep=True))
        sanitized_dataframe = restore_ordinal_labels(sanitized_dataframe_raw)
        try:
            features, prep_notifications = transform_single_record(preview_dataframe.copy(deep=True), context)
        except Exception as exc:
            return templates.TemplateResponse(
                "index.html",
                build_context(
        ...
        prediction["preview_html"] = preview_dataframe.to_html(index=False, classes="preview-table")
        prediction["preview_vertical_html"] = build_vertical_preview_html(preview_dataframe)
        combined_notifications = list(dict.fromkeys([*clean_notifications, *prep_notifications]))
        prediction["preprocessing_notes"] = combined_notifications
        prediction["source"] = "csv"
        prediction["generated_at"] = datetime.utcnow().isoformat()
        store.last_prediction = prediction

        storage_df = sanitized_dataframe.copy()
        ...
        record_to_save = sanitized_dataframe.iloc[0].to_dict()
    ...
    async def predict_from_form(request: Request) -> HTMLResponse:
        ...
        preview_record_df = pd.DataFrame([record])
        sanitized_record_raw, clean_notifications = clean_column_values(preview_record_df.copy(deep=True))
        sanitized_record_df = restore_ordinal_labels(sanitized_record_raw)
        try:
            features, prep_notifications = transform_single_record(preview_record_df.copy(deep=True), context)
        except Exception as exc:
            return templates.TemplateResponse(
                "index.html",
                build_context(
        ...
        prediction = prediction_summary(features, context)
        prediction["preview_html"] = preview_record_df.to_html(index=False, classes="preview-table")
        prediction["preview_vertical_html"] = build_vertical_preview_html(preview_record_df)
        combined_notifications = list(dict.fromkeys([*clean_notifications, *prep_notifications]))
        prediction["preprocessing_notes"] = combined_notifications
        ...
        storage_df = sanitized_record_df.copy()
        storage_df["predicted_anomaly"] = prediction["anomaly_label"]
        storage_df["is_suspicious"] = int(prediction["suspicious"])
        storage_df["analysis_run_at"] = datetime.utcnow().isoformat()
        ...
        record_to_save = sanitized_record_df.iloc[0].to_dict()
        record_to_save["predicted_class"] = prediction["anomaly_label"]
        record_to_save["class_probability"] = int(prediction["suspicious"])
...
#             )
#         preview_dataframe = dataframe.copy(deep=True)
#         sanitized_dataframe, _ = clean_column_values(dataframe.copy(deep=True))
#         try:
#             features, prep_notifications = transform_single_record(preview_dataframe, context)
#         except Exception as exc:
#             return templates.TemplateResponse(
#                 "index.html",
#                 build_context(
...
#         preview_record_df = pd.DataFrame([record])
#         sanitized_record_df, _ = clean_column_values(preview_record_df.copy(deep=True))
#         try:
#             features, prep_notifications = transform_single_record(preview_record_df, context)
#         except Exception as exc:
#             return templates.TemplateResponse(
#                 "index.html",
#                 build_context(
