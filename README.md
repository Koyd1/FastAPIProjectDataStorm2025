# Анализ аномалий на FastAPI SVWC
...
<!-- TODO:Римма Rima Переписать под результаты--> 
При успешной настройке Supabase каждое загруженное CSV (плюс предсказанные моделью столбцы) сохраняется в бакете. Перед сохранением данные очищаются от чувствительных полей (`transaction_id`, `customer_id`, `timestamp` и т.д.).

Все обезличенные записи объединяются в общую историю, поверх которой строятся:

- PCA-визуализация кластеров на основании случайного леса (Random Forest).
- Барчарт важности признаков, рассчитанный по обученному лесу.
- Распределение классов по всей истории загрузок.

Графики обновляются после каждой новой загрузки. Если Supabase недоступен, приложение всё равно держит историю в памяти в рамках текущего процесса.
